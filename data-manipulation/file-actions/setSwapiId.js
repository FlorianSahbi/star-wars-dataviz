...
fs.readdir(`./sourceDataSet/interactions`, (err, files) => {
    ...
    files.forEach((file, i) => {
        if (file !== '.git' && file !== 'starwars-full-interactions.json') {
            fs.readFile(`./targetDataSet/interactions/${file}`, (err, data) => {
                ...
                data.nodes.forEach((node, i) => {
                    if (node.api === null) {
                        nullChar.forEach(nc => {
                            if (nc.oldName === node.name) {
                                node.idSwapi = nc.idSwapi;
                                node.name = nc.name;
                            }
                        })
                    } else if (node.api !== null) {
                        if (node.api) {
                            node.idSwapi = Number(node.api.match(/[0-9]+/g)[0]);
                        } else {
                            console.log(`${node.api} ${node.name} ${file} pas d'api`)
                        }
                    }
                    node.interactionId = i;
                })
